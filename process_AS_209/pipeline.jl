...
Pipeline(;initial_input::Tuple = (), save_state::Bool = false) = begin
    if initial_input !== ()
        Pipeline(Stage[], initial_input, (), (), 0, Dict{Symbol, Any}(), Dict{String, Any}(), save_state)
    else
...
"""
macro context_save(call)
    @assert call.head === :call "Use as: @context_save f(dest, ...)"
    ...
    # Prebuild the function call with escaped callee and escaped args.
    call_expr = Expr(:call, esc(fn), dest_sym, map(esc, other_args)...)

    return quote
        # Evaluate destination exactly once
        local $(dest_sym) = $(esc(dest_expr))
        local $(res_sym) = $(call_expr)

        # Record into context["saved_files"]
        try
            if isdefined(Main, :context) && context !== nothing && context isa AbstractDict
                local _list = get!(context, "saved_files", String[])
                push!(_list, string($(dest_sym)))
                context["saved_files"] = _list
            else
                @warn "No Dict-like `context` available to log saved file."
            end
        catch _e
            @warn "Could not update context[\"saved_files\"]: $_e"
        end
